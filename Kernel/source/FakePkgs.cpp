...
void FakePkgs::InstallShellCorePatches()
{
	...
	if (p == nullptr)
	{
		kprintf("Failed to find the SceShellCore process?...\n");
		return;
	}
...
void FakePkgs::InstallShellUIPatches(proc* p)
{
	...
	size_t numModules;
	int handles[256];
	sys_dynlib_get_list(td, handles, 256, &numModules);

	// Make sure its not a sub proc.
	char sandboxPath[0x400];
	if (!GetSandboxPath(td, sandboxPath) || strstr(sandboxPath, "sce_sbx"))
	{
		kprintf("InstallShellUIPatches not ShellUI.\n");
		return;
	}
	...
	for (int i = 0; i < numModules; i++)
	{
		struct dynlib_info_ex info;
		info.size = sizeof(struct dynlib_info_ex);
		sys_dynlib_get_info_ex(p->p_threads.tqh_first, handles[i], 1, &info);

		if (strstr(info.name, "libkernel_sys.sprx"))
		{
...
{
	if (strstr(p->titleId, "NPXS20001"))
	{
		InstallShellUIPatches(p);
	}
}
