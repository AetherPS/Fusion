...
int elf_mapped_size(void* elf, size_t* msize) {
	...
	// check magic
	if (kernel_memcmp(ehdr->e_ident, ElfMagic, 4))
	{
...
int map_elf(void* elf, void* exec) {
	...
	if (phdr)
	{
		...
		for (int i = 0; i < ehdr->e_phnum; i++)
		{
			...
			if (phdr->p_filesz)
			{
				kernel_memcpy((uint8_t*)exec + phdr->p_paddr, (uint8_t*)elf + phdr->p_offset, phdr->p_filesz);
			}

			if (phdr->p_memsz - phdr->p_filesz)
			{
				kernel_memset((uint8_t*)exec + phdr->p_paddr + phdr->p_filesz, NULL, phdr->p_memsz - phdr->p_filesz);
			}
		}
	}
	else
	{
		...
		for (int i = 0; i < ehdr->e_shnum; i++)
		{
			...
			if (shdr->sh_size)
			{
				kernel_memcpy((uint8_t*)exec + shdr->sh_addr, (uint8_t*)elf + shdr->sh_offset, shdr->sh_size);
			}
		}
	}
